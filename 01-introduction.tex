...
The we prove that the well-known logical characterization of bisimilarity in terms of Hennessy-Milner Logic~\cite{HM80} (HML)
...
its variant considered here looks a suitable option to specify formulas 
...
\item we show that our assertions extend can be used to extend the Hennessy-Milner logic to our framework, in such a way that logical equivalence of processes coincide with bio-similarity;
...
\item we show that with a little coding effort, two RSs can
...
This paper is an extended version of paper~\cite{BBF19}.
