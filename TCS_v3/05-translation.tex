...
%  for each  $Cxt_s$ are
 %involved in the transition $t$;
  \item for each reaction $aj \in A$ and
  % each entity $s \in S$, 
  each virtual link offered by processes $P_a$ and $\mathit{Cxt}_1$ is  overlapped by exactly one solid link offered by  processes representing entities.
