...
% modeled by $A$. 
The set $A$ is the set of \emph{reactions} of $A$.
...
\begin{enumerate}
\item Let $a$ be a reaction. Then $a$ is enabled by $T$, denoted by $en_a(T)$, 
if $R_a \subseteq T$
and $I_a \cap T = \emptyset$. The result of $a$ on $T$, denoted by $res_a(T)$, 
is defined by:
$res_a(T) =P_a$, if $en_a(T)$, and $res_a(T) = \emptyset$ otherwise.
\item Let $A$ be a finite set of reactions. The result of $A$ on $T$, denoted 
