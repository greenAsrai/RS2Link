...
Here we focus on the encoding of the set of reactions $A \subseteq \mathit{rac}(S)$.
...
This schema will be later illustrated in detail in Example~\ref{ex:backbone}.
%\[
...
\mathit{Cxt}_n &\defeq &
%\left \{
^{cxt} \backslash  \blockchain{\widehat{s}_i}{\widehat{s}_o}{s\!\in \!C_n}  \link{}{} \blockchain{\underline{e}_i}{\underline{e}_o}{e \!\not\in \! C_n}   \backslash_{p_1}. \mathit{Cxt}_{n+1}
%+ \\
...
\triangleq
 \restrict{\mathit{names}}\left(I ~|~ \prod_{a\in A} P_a ~|~  \mathit{Cxt}_1 ~|~\prod_{s\in C_0}P_{s} ~|~ \prod_{s \notin C_0}\overline{P_s}\right)
$$
...
For technical reasons, we introduce the (trivially recursive) init  process $I \defeq \link{\silent}{r_1}.I$: it is needed
to allow the name $r_1$ to be matched at the start of any chain at any instant of time.
\end{definition}
...
This is due to the fact that  all the channel names $r_j$, $p_j$, $\mathit{cxt}$, including those for decorated names $s_{i},s_{o},\overline{s}_i,\overline{s}_o,...$ are  restricted.
...
%  for each  $Cxt_s$ are
 %involved in the transition $t$;
  \item for each reaction $aj \in A$ and each entity $s \in S$, each virtual link offered by processes $P_a$ and $\mathit{Cxt}_1$ is  overlapped by exactly one solid link offered by  processes representing entities.
...
I~|~ \prod_{a\in A} P_a ~|~  \mathit{Cxt}_1~|~\prod_{s\in C_0}P_{s} ~|~ \prod_{s \notin C_0}\overline{P_s} .
$$
...
\begin{example}\label{ex:backbone}
 Let ${\cal A}$ be a RS whose specification contains two entities, $s1$ and $s2$, and the reactions $r_1=(s1,\emptyset , s2)$ and $r_2 = (s2, \emptyset , s1)$ that produce $s2$ if $s1$ is present and $s1$ if $s2$ is present. For simplicity, we consider empty sets of inhibitors, which are not allowed by Definition~\ref{def:reaction}, but the reader can assume a void inhibitor is present in both reactions. Then, we assume an  extended interactive process $\pi=(\gamma,\delta)$ where the context $\gamma$ provides  $s1$ and $s2$ at every step, but we assume that only $s1$ is initially present. Since the context sequence is constant, we omit the subscript from $\mathit{Cxt}$. The corresponding c\CNA \ process is $\llbracket{\cal A},\gamma\rrbracket \defeq \restrict{\mathit{names}} \llparenthesis{\cal A},\gamma\rrparenthesis$, with 
$$
\llparenthesis{\cal A},\gamma\rrparenthesis \defeq 
...
Our transition labels are quite verbose; then, to simplify their processing, we introduce a function that takes a solid link chain and returns a simple string by eliminating all the channel matching pairs and leaving just one placeholder for them. This transformation is harmless, in the sense that it retains all the information in the chain, because it is applied to complete chains only. 
...
\end{array}\\
\]
where the usual string concatenation if represented by juxtaposition.
...
$$P  = \restrict{\mathit{names}} \left(I~|~\prod_{a \in A}P_a ~|~ \mathit{Cxt}_1 ~|~ \prod_{s \in C_0}P_s ~|~ \prod_{s \notin C_0}\overline{P}_s\right).
$$
 ...
 \item
  ...
  \item 
 $P'= \restrict{\mathit{names}} \left(I~|~\prod_{a \in A}P_a~|~ \mathit{Cxt}_2 ~|~ \prod_{s \in C_1 \cup D_1}P_s| \prod_{s \notin C_1 \cup D_1}\overline{P}_s\right)$.
 \end{enumerate}
