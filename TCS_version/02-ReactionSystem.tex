...
A reaction over $S$ is a triplet $a = (R,I,P)$, where $R, I, P$ are finite,  
...
such that $S$ is a finite set of  entities, and $A \subseteq rac(S)$ is a set of reactions over $S$.
...
\begin{enumerate}
\item Let $a$ be a reaction over $S$. Then $a$ is enabled by the entities in the set $W$, denoted by $\mathit{en}_a(W)$, 
...
\item Let $A$ be a finite set of reactions over $S$. The result of $A$ on $W$, denoted 
...
%\noindent
where 
$ C_{i}, D_{i}  \subseteq S$ are set of entities for any $i\in[0,n]$, $D_0 = \emptyset$, and 
